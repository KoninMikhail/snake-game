...
export const ExtraLargeCaption = styled.span<ITextProps>`
    font-size: ${({ theme }) => theme.typography.fontSize.caption.xl.mobile};
    font-weight: ${({ weight }) => weight ?? 'normal'};

    ${({ align }) => align && `text-align: ${align};`}
    ${up('md')} {
        font-size: ${({ theme }) => theme.typography.fontSize.caption.xl.tablet};
    }

    ${up('xl')} {
        font-size: ${({ theme }) => theme.typography.fontSize.caption.xl.desktop};
    }
`;
